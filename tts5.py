...
class TtsManager:
    def __init__(self):
        self.client = None
        self.ws = None
        self.interrupt_ws = None  # Separate WebSocket for interruptions
        self.p = None
        self.stream = None
        self.voice_id = None
        self.model_id = None
    ...
    def initialize_tts(self):
        ...
        self.stream = self.p.open(
            ...
            start=True  # Ensure the stream is started immediately
        )
        ...
        # Create a separate WebSocket for interruptions
        self.interrupt_ws = self.client.tts.websocket()
    ...
    def speak(self, text):
        ...
        self.text_queue.put((text, None, time.time(), 1, False))  # Normal priority, use main websocket
    ...
    def interrupt_and_speak(self, text):
        ...
        # Queue the new text with highest priority and flag to use interrupt websocket
        self.text_queue.put((text, new_context_id, request_time, 10, True))  # Priority 10, use interrupt websocket
                ...
                if ctx_id:
                    try:
                        ...
                        try:
                            ...
                            # Immediately remove from client tracking without waiting for response
            ...
            try:
                ...
                while not self.text_queue.empty():
                    try:
                        ...
                        text, context_id, request_time, priority, use_interrupt_ws = item
                ...
                if highest_priority_item:
                    text, context_id, request_time, priority, use_interrupt_ws = highest_priority_item
                    ...
                    try:
                        ...
                        with self.context_lock:
                            if new_context_id not in self.active_contexts:
                        ...
                        # For high priority requests (interruptions), reduce initial buffering
                        if priority >= 10:
                            self.min_initial_frames = 0  # No buffering for interruptions
                        else:
                            self.min_initial_frames = 1  # Normal buffering for regular speech
                        
                        # Choose which WebSocket connection to use
                        ws_to_use = self.interrupt_ws if use_interrupt_ws else self.ws
                        
                        # Generate and stream audio
                        for chunk in ws_to_use.send(
    ...
    def shutdown(self):
        ...
        if self.interrupt_ws:
            self.interrupt_ws.close()

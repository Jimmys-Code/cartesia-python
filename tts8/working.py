...
class TenTtsManager:
    def __init__(self):
        ...
        # We removed the housekeeper logic, so no housekeeping thread
        # The only refresh now happens when we switch away from a connection in an interrupt.

        # Start everything
        self.initialize_tts()

    def initialize_tts(self):
        """Initialize TTS system, including all ten WebSocket connections in parallel."""
        ...
        # Establish all 10 connections simultaneously
        threads = []
        for label in self.conn_labels:
            t = threading.Thread(target=self._init_connection, args=(label,))
            threads.append(t)
            t.start()
    ...
    def _init_connection(self, label):
        """Helper method to initialize a single WebSocket connection in a dedicated thread."""
        print(f"Establishing WebSocket connection {label} in parallel...")
    ...
    def interrupt_and_speak(self, text):
        ...
        After switching away, we refresh the old connection so it will be ready again later.
        """
        ...
        for _ in range(10):
            ...
            if next_label is not None:
                break
            # If none is ready, wait a bit so the background refresh can finish
            time.sleep(0.3)
